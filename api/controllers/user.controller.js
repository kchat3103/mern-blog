...
export const deleteUser = async(req, res, next) =>{
  if(!req.user.isAdmin && req.user.id!==req.params.userId){ //only admin can delete the user
...
export const getUsers = async(req,res,next)=>{
  ...
  try {
    ...
    const usersWithoutPassword = users.map((user)=>{ //get the users without password
    ...
    const lastMonthUsers = await User.countDocuments({
      createdAt:{$gte:oneMonthAgo} //sorted by greater than one month ago
    });
